...
double evaluate(struct boardVar board, int turn, int maxTurn, double probabilities[6]){
  // aka minmax
  ...
  if(turn >= maxTurn || board.game_ended){
    ...
    if(board.player)
      for(int i = 0; i<6; i++)
        eval+=board.a[i][board.player]*probabilities[i];
    else
     for(int i = 0; i<6; i++)
        eval-=board.a[i][board.player]*probabilities[i];
  ...
  for(int i = 0; i<6; i++){
    if(board.a[i][board.player]){
      ...
      double temp = evaluate(newBoard, turn+1, maxTurn, probabilities);
...
struct positionVar engineMove(struct boardVar board, int evalLimit, double probabilities[6]){
...
void playPvE(struct boardVar board, bool player, int evalLimit, double probabilities[]){
  struct positionVar position;
  int column = -2;
  ctr = 0;
  board = checkEnd(board);
  if(!board.game_ended){
    ...
    if(board.player != player){
      printf("\nThinking...\n");
      board = movePieces(board, engineMove(board, evalLimit, probabilities));
    }
    else{
    ...
    playPvE(board, player, evalLimit, probabilities);
  } else {
    printf("\nGame ended: %d - %d\n", board.s[0], board.s[1]);
...
int main(){ 
  ...
  double probabilities[6] = {p1, p2, p3, p4, p5, p6};
  ...
  playPvE(newBoardVar, 0, 9, probabilities);
